...
export default function VendorOrderDetail() {
  ...
  const { data: orderResp, isLoading, error, refetch } = useApiQuery(
    () => vendorApi.getVendorOrderById(id!), [id], { enabled: !!id }
  );
  ...
  if (!order) return <div className="flex items-center justify-center h-64"><div className="text-center"><p className="text-lg font-medium">Order not found</p><Button variant="link" onClick={() => navigate("/vendor/orders")}>Back to Orders</Button></div></div>;
            ...
            <CardContent>
              {(order.items || []).map((item: any, i: number) => (
                <div key={i} className="flex items-center gap-4 py-3 border-b last:border-0">
                  ...
                  <div className="flex-1 min-w-0"><p className="font-medium text-sm truncate">{item.productName}</p><p className="text-xs text-muted-foreground">Qty: {item.quantity}</p></div>
                  <p className="font-semibold text-sm">₹{(item.price * item.quantity).toLocaleString("en-IN")}</p>
                </div>
              ))}
              <Separator className="my-3" />
              <div className="flex justify-between items-center"><span className="text-sm text-muted-foreground">Total Amount</span><span className="text-lg font-bold">₹{order.total?.toLocaleString("en-IN")}</span></div>
            </CardContent>
          </Card>
              ...
              <div className="space-y-4">
                {timeline.map((step, i) => (
                  <div key={i} className="flex gap-3">
                    ...
                    <div className="pb-4"><p className={`text-sm font-medium ${step.done ? "" : "text-muted-foreground"}`}>{step.label}</p>{step.date && <p className="text-xs text-muted-foreground">{new Date(step.date).toLocaleString("en-IN")}</p>}</div>
                  </div>
                ))}
              </div>
            </CardContent>
        ...
        <div className="space-y-6">
          <Card className="shadow-card">
            ...
            <CardContent className="space-y-3">
              ...
              {order.trackingId && <div className="text-sm"><span className="text-muted-foreground">Tracking: </span><code className="font-mono">{order.trackingId}</code></div>}
            </CardContent>
          </Card>
          <Card className="shadow-card">
            ...
            <CardContent><p className="text-sm">{order.shippingAddress}</p></CardContent>
          </Card>
          <Card className="shadow-card">
