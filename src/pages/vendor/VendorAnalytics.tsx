...
const salesData = [
  { day: "Mon", sales: 12400, orders: 18, visitors: 890 },
  { day: "Tue", sales: 18900, orders: 24, visitors: 1120 },
  { day: "Wed", sales: 15600, orders: 21, visitors: 980 },
  { day: "Thu", sales: 22100, orders: 29, visitors: 1340 },
  { day: "Fri", sales: 28400, orders: 35, visitors: 1560 },
  { day: "Sat", sales: 34200, orders: 42, visitors: 1890 },
  { day: "Sun", sales: 19800, orders: 26, visitors: 1230 },
];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard title="Total Orders" value="195" change="+9.8%" changeType="positive" icon={ShoppingCart} />
        <StatCard title="Revenue" value="₹1.51L" change="+18.5%" changeType="positive" icon={DollarSign} />
          ...
          <CardContent className="space-y-3">
            {trafficSources.map((s, i) => (
              <div key={s.source} className="flex items-center gap-3">
                ...
                <Badge variant="outline" className={`text-[10px] shrink-0 ${s.change.startsWith("+") ? "text-primary" : "text-destructive"}`}>
                  {s.change}
                </Badge>
              </div>
            ))}
              ...
              <TableBody>
                {topProducts.map((p, i) => (
                  <TableRow key={p.id}>
                    ...
                    <TableCell className="text-right text-sm">{Math.round(p.reviewCount / 10)}</TableCell>
                    <TableCell className="text-right text-sm font-medium">₹{(p.price * Math.round(p.reviewCount / 10)).toLocaleString("en-IN")}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
