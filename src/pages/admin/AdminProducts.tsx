...
export default function AdminProducts() {
  ...
  const products = (productsResp?.data ?? productsResp ?? []) as any[];
  const filtered = products.filter((p: any) => p.name.toLowerCase().includes(search.toLowerCase()));
  const { page, setPage, totalPages, paginatedItems, totalItems } = usePagination(filtered, 8);

  if (isLoading) return <div className="space-y-6"><h1 className="font-display text-xl font-bold">Product Moderation</h1><TableSkeleton rows={6} cols={6} /></div>;
  if (error) return <div className="space-y-6"><h1 className="font-display text-xl font-bold">Product Moderation</h1><PageError message={error} onRetry={refetch} /></div>;
