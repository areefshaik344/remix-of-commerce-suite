...
export default function AdminProductDetail() {
  ...
  const { data: productResp, isLoading, error, refetch } = useApiQuery(
    () => adminApi.getProductById(id!), [id], { enabled: !!id }
  );
  const product = productResp?.data ?? productResp;

  const { data: reviewsResp } = useApiQuery(
    () => reviewApi.getProductReviews(id!), [id], { enabled: !!id }
  );
  const productReviews = (reviewsResp?.data ?? reviewsResp ?? []) as any[];

  if (isLoading) return <DashboardSkeleton />;
  if (error) return <PageError message={error} onRetry={refetch} />;
  if (!product) return <div className="flex items-center justify-center h-64"><div className="text-center"><p className="text-lg font-medium">Product not found</p><Button variant="link" onClick={() => navigate("/admin/products")}>Back</Button></div></div>;

  const handleApprove = () => { setApprovalStatus("approved"); setShowReject(false); toast({ title: "Product approved", description: `"${product.name}" is now live.` }); };
  const handleReject = () => {
    if (!rejectionNote.trim()) { toast({ title: "Reason required", variant: "destructive" }); return; }
    setApprovalStatus("rejected"); setShowReject(false);
          ...
          <Card className="shadow-card">
            ...
            <CardContent className="space-y-4">
              ...
              {product.tags?.length > 0 && (
                <div className="flex flex-wrap gap-1.5">{product.tags.map((t: string) => <Badge key={t} variant="outline">{t}</Badge>)}</div>
              )}
            </CardContent>
          </Card>
              ...
              <CardContent className="space-y-3">
                {product.variants.map((v: any, i: number) => (
                  <div key={i}>
                    ...
                    <div className="flex flex-wrap gap-1">{v.options.map((o: string) => <Badge key={o} variant="outline" className="text-xs">{o}</Badge>)}</div>
                  </div>
                ))}
              </CardContent>
            </Card>
